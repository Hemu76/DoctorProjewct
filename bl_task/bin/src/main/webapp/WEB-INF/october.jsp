...
{
<%
List<Integer> ll =(List<Integer>)request.getAttribute("olist");
for(Integer d:ll)
{
%>
...
function slots(name)
{
	var flag=0;
<%
	List<Integer> l =(List<Integer>)request.getAttribute("olist");
	for(Integer d:l)
	{
	%>
		if(name==<%=d%>)
	...
	<%}%>
	if(flag==0)
		window.location.href = "slot?button=" + encodeURIComponent(name)+"&p="+encodeURIComponent("oct");
